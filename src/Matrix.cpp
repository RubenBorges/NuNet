    ...
    const Matrix& bias)
{
    if (input.cols_ != weights.rows_)
        throw std::invalid_argument(
            "Dense input/weight dimensions do not match");

    if (bias.rows_ != 1 ||
        bias.cols_ != weights.cols_)
    {
        throw std::invalid_argument(
            "Dense bias dimensions do not match");
    }
    ...
    std::size_t stride) const
{
    ...
    {
        throw std::invalid_argument(
            "Pool window is larger than matrix");
    }
...
Matrix Matrix::convolve(
    ...
    std::size_t stride) const
{
    if (kernel.rows_ > rows_ ||
        kernel.cols_ > cols_)
    {
        throw std::invalid_argument(
            "Kernel cannot be larger than matrix");
    }
    ...
    const std::size_t output_rows =
        (rows_ - kernel.rows_) / stride + 1;

    const std::size_t output_cols =
        (cols_ - kernel.cols_) / stride + 1;
