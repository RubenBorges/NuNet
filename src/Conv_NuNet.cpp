...
int main() {
    ...
    // 1. INPUT IMAGE (34x26 pixels, 1 channel)
    const int INPUT_ROWS = 34;
    const int INPUT_COLS = 26;
    ...
    std::cout << "--- Forward Pass Simulation for 34x26 Thermal Image ---" << std::endl;
    ...
    int conv_rows = (INPUT_ROWS - KERNEL_SIZE) / STRIDE + 1; // (34 - 3) + 1 = 32
    int conv_cols = (INPUT_COLS - KERNEL_SIZE) / STRIDE + 1; // (26 - 3) + 1 = 24
    Matrix conv_output(conv_rows, conv_cols);

    // Perform Convolution + ReLU Activation
    for (int r = 0; r < conv_rows; ++r) {
        for (int c = 0; c < conv_cols; ++c) {
            double sum = 0.0;
    ...
    int pool_rows = conv_rows / POOL_STRIDE; // 32 / 2 = 16
    int pool_cols = conv_cols / POOL_STRIDE; // 24 / 2 = 12
    Matrix pool_output(pool_rows, pool_cols);

    for (int r = 0; r < pool_rows; ++r) {
        for (int c = 0; c < pool_cols; ++c) {
